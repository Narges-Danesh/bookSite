...
let decrement = (id) => {
  ...
  if (search.item === 1) {
    document.getElementById(id).classList.add("remove-book");
    let removeBook = document.querySelector(".remove-book");
    let animationDur = getComputedStyle(removeBook).animationDuration;
...
let update = (id) => {
  ...
  if (!detailsPage) {
    ...
    totalPrice.innerHTML = thisBook.price * search.item;
...
function totalPriceShow() {
  let totalPrices = document.querySelectorAll(".cart-book-total-price");
  let sum = 0;
  totalPrices.forEach((price) => (sum += +price.innerHTML));

  let deliveryPriceContainer = document.querySelector(".delivery-price");
  let deliveryPrice = deliveryPriceContainer.children[1].innerHTML;
  let itemsPriceContainer = document.querySelector(".items-price");
  let totalPriceContainer = document.querySelector(".total-price");
  itemsPriceContainer.children[1].innerHTML = `${sum} تومان`;
  ...
  quantityContainer.children[1].innerHTML = total;
}
let calculation = (id) => {
  let total = 0;
  basket.forEach((x) => {
